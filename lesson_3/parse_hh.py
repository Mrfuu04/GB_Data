        ...
        while True:
            ...
            for vacancy in item:
                ...
                try:
                    ...
                    if price.startswith('от'):
                        checker = price.replace(' ', '')
                        reward = {'min': int(checker.split()[1]), 'max': None, 'valute': checker.split()[-1]}
                    elif price.startswith('до'):
                        checker = price.replace(' ', '')
                        reward = {'min': None, 'max': int(checker.split()[1]), 'valute': checker.split()[-1]}
                    else:
                        checker = price.replace(' ', '').split('–')
                        min_price = checker[0]
                        max_price = checker[1].split()[0]
                        valute = checker[1].split()[1]
                        reward = {'min': int(min_price), 'max': int(max_price), 'valute': valute}
                except AttributeError:
                    reward = {'min': None, 'max': None, 'valute': None}
                main_data['Название'] = title
                main_data['Зарплата'] = reward
